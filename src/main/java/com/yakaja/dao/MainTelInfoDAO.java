...
public class MainTelInfoDAO {
	...
	public boolean writeReview(String head, String mem_id, String body, double score, String hotelName) {
	...
	public ArrayList<RoomVO> getRoomInfo(String tmp, String selected_in, String selected_out, String hotel_name)
		...
		String sql = "select distinct r.*" + " from " + table + " r" + " where r.room_num not in("
			    ...
			    + " = (select hotel_id from pr2hotel where hotel_name = ?)"
				+ " and (to_char(to_date(substr(v2.pr2record,8,6),'YYMMDD'), 'YYYY-MM-DD') >= ?"
				+ " and to_char(to_date(substr(v2.pr2record,8,6),'YYMMDD'), 'YYYY-MM-DD') < ?))"
				+ " order by r.room_num";
		pstmt = con.prepareStatement(sql);
		pstmt.setString(1, hotel_name);
	...
	public boolean getReservHotel(String hotel_id, int room_num, String reserv_in, String reserv_out, String mem_id,
		...
		int dateDiff = Period.between(startDate, endDate).getDays();
	...
	public ArrayList<ReservVO> findReserv(String mem_id) {
		...
		String sql = "select * from pr2reserv where mem_id = ? order by reserv_id";
	...
	public ArrayList<String> findReservHotelName(String mem_id) throws SQLException {
		
		ArrayList<String> stList = new ArrayList<String>();
		String sql = "select r.reserv_id, h.hotel_id, h.hotel_name from pr2reserv r"
	            + " inner join pr2hotel h"
	            + " on r.hotel_id = h.hotel_id"
	            + " where r.mem_id = ?"
	            + " order by r.reserv_id";
	...
	//-----
	  public boolean resvDelete(int reserv_id) {
	...
	// 회원탈퇴
	  ...
	  public boolean delete(String mem_id) {
		...
		public boolean isValidPassword(String mem_id, String mem_pw) {
			try {
				// 사용자 정보 가져오기
				MemberVO tidao1 = getUserById(mem_id);

				// 비밀번호 검증
				System.out.println("return 값 : " + tidao1.getMem_pw().equals(mem_pw));
				return tidao1.getMem_pw().equals(mem_pw);
			} catch (Exception e) {
				// 예외 처리
				e.printStackTrace();
		...
		public MemberVO getUserById(String mem_id) {
			MemberVO tidao1 = null;
			String sql = "select * from pr2member where mem_id = ?";
			try {
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, mem_id);
	...
	public ArrayList<HotelVO> getHotelList(String mem_id) throws SQLException{
		
		ArrayList<HotelVO> list = new ArrayList<HotelVO>();
		
		String sql = "select * from pr2hotel where mem_id = ?";
		pstmt = con.prepareStatement(sql);
		pstmt.setString(1, mem_id);
	...
	public ArrayList<ReservVO> getReservList(String hotel_id) throws SQLException{
		
		ArrayList<ReservVO> list = new ArrayList<ReservVO>();
		String sql = "select * from pr2reserv where hotel_id = ?";
		pstmt = con.prepareStatement(sql);
		...
		while(rs.next()) {
			...
			reservVO.setRoom_price(rs.getInt("room_price"));
			reservVO.setHotel_id(rs.getInt("hotel_id"));
			reservVO.setReserv_in(rs.getString("reserv_in"));
			reservVO.setReserv_out(rs.getString("reserv_out"));
			reservVO.setMem_id(rs.getString("mem_id"));
	...
	public ReservVO getReservOne(int reserv_id) throws SQLException {
	...
	public boolean deleteReserv2(int hotel_id, int room_num, String reserv_in, String reserv_out) throws SQLException {
		
		String sql = "delete from pr2reserv2"
				+ " where substr(pr2record, 1, 4) = ?"
				+ " and substr(pr2record, 5, 3) = ?"
				+ " and (to_char(to_date(substr(pr2record,8,6),'YYMMDD'), 'YYYY-MM-DD') >= ?"
				+ " and to_char(to_date(substr(pr2record,8,6),'YYMMDD'), 'YYYY-MM-DD') < ?)";
		...
		pstmt.setString(3, reserv_in);
		pstmt.setString(4, reserv_out);
