            ...
            <div class="navbar-nav ms-auto p-4 p-lg-0">
	            <c:if test="${not empty sessionScope.memberVO}">
	            	<c:if test="${sessionScope.memberVO.mem_admin == 0}">
	                	 ...
	                	 <a href="jaksungReview.jsp" class="nav-item nav-link">리뷰작성</a>
	            	</c:if>
	                <c:if test="${sessionScope.memberVO.mem_admin == 1}">
	            		 <a href="hotelJoinForm.do" class="nav-item nav-link">호텔등록</a>
	            		 <a href="hostList.do?mem_id=${sessionScope.memberVO.mem_id}" class="nav-item nav-link">등록호텔확인</a>
	                	 <a href="hostReservList.do?mem_id=${sessionScope.memberVO.mem_id}" class="nav-item nav-link">예약목록확인</a>
	                	 <a href="myInfo.do" class="nav-item nav-link">내정보</a>
	            	</c:if>
	            	<c:if test="${sessionScope.memberVO.mem_id == 'admin'}">
	            		 <a href="adminMember.do" class="nav-item nav-link">회원목록</a>
	                	 <a href="adminHotel.do" class="nav-item nav-link">호텔목록</a>
	                	 <a href="adminReserv.do" class="nav-item nav-link">예약목록</a>
	            	</c:if>
	            </c:if>
	            <a href="paging.do" class="nav-item nav-link">리뷰게시판</a>
